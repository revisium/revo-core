...
import { createAgentAttemptExecutionAdapter } from '../../../src/infrastructure/run-runtime/agent-runtime-adapter/revo-agent-runtime-port.js';

const unavailable = (): never => {
  throw new Error('Agent runtime is unavailable.');
};

const unavailableAgentPort: AgentAttemptExecutionPort = {
...
export const definition: AgentDefinitionInput = {
  ...
  id: 'test-agent',
  version: '1.0.0',
  installationId: 'test-agent-installation',
...
export const setup = async (_runtime: RuntimeMock, options = createOptions()) => {
  ...
  const port = createAgentAttemptExecutionAdapter({
    ...
    launchContext: {
      environment: {
        inherit: ['HOME', 'PATH'],
        variables: { REVO_TEST_MODE: 'true' },
        secrets: { SHARED_SECRET: 'launch-context-secret' },
      },
    },
    manager,
  });
