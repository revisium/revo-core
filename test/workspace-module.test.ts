...
describe('Workspace module and transports', () => {
  ...
  test('connects a nonversioned Folder, trims name, records actor and preserves Project records', async () => {
    ...
    const workspace = await connect('  Local files  ', source, 'folder', createdProject.projectId);
  ...
  test('bounded text reads enforce a separate capability, deny rules and size limits', async () => {
    const fs = app.get(FileSystemApiService);
  ...
  test('lists connected Workspaces by name then id and projects the first three plus total count', async () => {
    ...
    const records = await Promise.all(
      ['Delta', 'Bravo', 'Alpha', 'Charlie'].map((name) => connect(name)),
    );
  ...
  test('REST and GraphQL share Workspace CRUD, errors and Project summary', async () => {
    ...
    const created = await request(app.getHttpServer())
      ...
      .expect(201);
    const id: string = created.body.workspaceId;
