...
describe('Persisted filesystem access configuration', () => {
  ...
  test('intersects agent grants, Workspace rules and the trusted root for all operations', async () => {
    const client = filesystem.withAccess(() => policies.resolveAccess(selection));
    expect((await client.getRoots({})).edges.map((edge) => edge.node.path)).toEqual([workspace]);
    expect(
      (await client.getDirectory({ path: workspace, includeHidden: true })).entries.edges.map(
        (edge) => edge.node.name,
      ),
    ).toEqual(['.git', 'docs']);
    ...
    await expect(
      client.createDirectory({ parentPath: workspace, name: 'denied' }),
  ...
  test('policy updates and revocation affect an already-bound client on its next call', async () => {
    const client = filesystem.withAccess(() => policies.resolveAccess(selection));
    expect(await client.exists({ path: workspace })).toBe(true);
    ...
    await client.createDirectory({ parentPath: workspace, name: 'created' });
    await expect(
      client.createDirectory({ parentPath: path.join(workspace, '.git'), name: 'denied' }),
    ).rejects.toMatchObject({ code: 'FILE_SYSTEM_PERMISSION_DENIED' });
    await policies.revokePolicy({ id: selection.grantPolicyId, expectedVersion: 2 });
    await expect(client.exists({ path: workspace })).rejects.toMatchObject({
      code: 'FILE_SYSTEM_PERMISSION_DENIED',
    });
  ...
  test('a step policy narrows agent write permissions to docs', async () => {
    ...
    const client = filesystem.withAccess(() =>
      policies.resolveAccess({ ...selection, restrictionPolicyIds: [stepId] }),
    );
    ...
    await expect(client.getRoots({})).rejects.toMatchObject({
      code: 'FILE_SYSTEM_PERMISSION_DENIED',
    });
  });
  ...
  test('host restrictions and canonical paths also constrain persisted policies', async () => {
    ...
    const client = filesystem.withAccess(() => policies.resolveAccess(selection));
    await expect(
      client.getDirectory({ path: path.join(workspace, 'external') }),
    ).rejects.toMatchObject({ code: 'FILE_SYSTEM_PERMISSION_DENIED' });
    const host = FileSystemAccessContext.create({
      scopes: [{ rootPath: path.join(workspace, 'docs'), allow: allPermissions }],
    });
    const narrowed = filesystem.withAccess(() =>
      policies.resolveAccess({ ...selection, authority: host }),
    );
    await expect(narrowed.getEntry({ path: workspace })).rejects.toMatchObject({
      code: 'FILE_SYSTEM_PERMISSION_DENIED',
    });
    expect(await narrowed.isDirectory({ path: path.join(workspace, 'docs') })).toBe(true);
  });
  ...
  test('policy storage failures reject access without leaking database errors', async () => {
    ...
    await expect(client.getEntry({ path: workspace })).rejects.toMatchObject({
