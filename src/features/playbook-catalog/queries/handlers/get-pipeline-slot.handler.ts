import { NotFoundException } from '@nestjs/common';
import { QueryHandler, type IQueryHandler } from '@nestjs/cqrs';
import { EngineApiService } from '@revisium/engine';

import { CatalogDraftService } from '../../catalog-draft.service.js';
import { CatalogError, CatalogTable } from '../../constants/catalog.constants.js';
import {
  GetPipelineSlotQuery,
  type GetPipelineSlotQueryReturnType,
} from '../impl/get-pipeline-slot.query.js';

@QueryHandler(GetPipelineSlotQuery)
export class GetPipelineSlotHandler implements IQueryHandler<
  GetPipelineSlotQuery,
  GetPipelineSlotQueryReturnType
> {
  constructor(
    private readonly drafts: CatalogDraftService,
    private readonly engine: EngineApiService,
  ) {}

  async execute({ data }: GetPipelineSlotQuery): Promise<GetPipelineSlotQueryReturnType> {
    const { revisionId, isHead } = await this.drafts.resolveRevision(data);
    const row = await this.engine.getRow({
      revisionId,
      tableId: CatalogTable.pipelineSlots,
      rowId: data.id,
    });

    if (row === null) {
      throw new NotFoundException(CatalogError.recordUnavailable);
    }

    return this.drafts.toRecord(row, revisionId, isHead, CatalogTable.pipelineSlots);
  }
}
