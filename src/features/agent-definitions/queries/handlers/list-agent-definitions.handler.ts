...
import { paginateAgentDefinitions } from '../../contracts/agent-definitions.pagination.js';
import {
...
> {
  ...
  async execute({ data }: ListAgentDefinitionsQuery): Promise<ListAgentDefinitionsQueryReturnType> {
    return paginateAgentDefinitions(this.manager.sessions.listAgents(), data, (record) =>
      JSON.stringify([record.agent.id, record.agent.version]),
    );
  }
}
