...
} from '../../features/file-system/contracts/file-system.contracts.js';
...
export class FileSystemBrowserAccessService {
  ...
  async getContext(): Promise<FileSystemAccessContext> {
    ...
    if (!policyId || roots.length === 0) {
      return FileSystemAccessContext.create({ scopes: [] });
    }
    ...
    const authority = FileSystemAccessContext.create({
