...
export interface CreateAgentAttemptExecutionAdapterOptions {
  ...
  readonly launchContext: AgentStartContext;
}
    ...
    start: async (input, context) => {
      ...
      try {
        const descriptor = manager.getAgent({
          ...
          installationId: input.binding.pin.installationId,
        });
        ...
        const handle = await manager.start(
          runtimeRequest(input, allocation.absolutePath),
          runtimeEnvironment(
            invocationOwner.credentials,
            input.binding,
            signal,
